...
<title>修改客户信息</title>
    ...
    $(function () {
    	...
    	var length,layout,layouts,buttonCSS = "margin-top:20%;width:80%;height:5%;border-radius: 5px;border:none;";
		var myLayout = new dhtmlXLayoutObject("see", "1C");
		myLayout.cells("a").hideHeader();
		myLayout.cells("a").setHeight(2800);
		...
		function bar(e){
			 var box = document.getElementById("seeBox");
			  box.style.cssText = "width:83%;height:2770px;margin-left:12%;margin-top:1%;margin-bottom:3%;background: #ffffff;box-shadow: 0 0 10px #000000;";	
		...
		function localData(){
			if(localStorage.infos){
				var aa = JSON.parse(localStorage.infos);
				
				
				$("input[name='cCusCode']")[0].setAttribute("readonly","readonly");					
				$("input[name='cCusCode']")[0].value = aa.cCusCode;
				$("input[name='cCusName']")[0].value = aa.cCusName;
				$("input[name='cCusAddress']")[0].value = aa.cCusAddress;
				$("input[name='cCusAddName']")[0].value = aa.cCusAddName;
				$("input[name='cSupUnit']")[0].value = aa.cSupUnit;
				$("input[name='cCusPhone']")[0].value = aa.cCusPhone;
				$("input[name='cCusBussinse']")[0].value = aa.cCusBussinse;
				$("input[name='cCusHomePage']")[0].value = aa.cCusHomePage;
				$("input[name='cMail']")[0].value = aa.cMail;
				$("input[name='dCusSale']")[0].value = aa.dCusSale;
				$("input[name='iStaffCount']")[0].value = aa.iStaffCount;
				$("input[name='cCusFax']")[0].value = aa.cCusFax;
				$("input[name='dCapital']")[0].value = aa.dCapital;
				$("textarea[name='cMemo']")[0].value = aa.cMemo;
				$("input[name='cCCCode']")[0].parentNode.children[0].value = aa.cCCCode;
				$("input[name='cDepCode']")[0].parentNode.children[0].value = aa.cDepCode;
				$("input[name='cCusVIP']")[0].parentNode.children[0].value = aa.cCusVIP;
				$("input[name='cInfoSource']")[0].parentNode.children[0].value = aa.cInfoSource;
				$("input[name='cDCCode']")[0].parentNode.children[0].value = aa.cDCCode;
				$("input[name='cPsnCode']")[0].parentNode.children[0].value = aa.cPsnCode;
				$("input[name='cTrade']")[0].parentNode.children[0].value = aa.cTrade;
				
				var att1 = ',cName,cClub,cDep,cPost,cPhone,cMoblePhone,cQQ,cMoblePhone2,cZR,cBridthday,cAge,cSex,cLike,cMove,cFriend,cEmail,cState';
				addGrid(layout.lays[0].cells("c").getAttachedObject(),'/customer/selcustomerpeople.action',att1);
				
				addGrid(layout.lays[0].cells("d").getAttachedObject(),'/customer/selcustomeraddress.action',',cSupUnit,cCusAddress,isDefault');
				
				addGrid(layout.lays[0].cells("e").getAttachedObject(),'/customer/selcustomerassociation.action',',cCompanyName,cAssociation');
				
				addGrid(layout.lays[0].cells("h").getAttachedObject(),'/customer/selcustomereffective.action',',cid,cProductName,cStartDate,cEffective,cEndDate,note');
			}
		}
		
		//载入联系人，关联公司，地址等
		function addGrid(grid,urls,att1){
			grid.clearAll(false);
			$.ajax({
				   type: 'POST',
				   url: urls,
				   data: {groupID:window.top.getGroupID(),cCusCode:localStorage.customerID},
				   success: function(r){
					   var array = new Array();
					     var att = att1;
						 var arr2 = att.split(',');
		...
		$('#postBar').scroll(function(event){
		 ...
		 myToolbar.addItems([		    
		        ...
		        {ID: "close", Type: "button", Text: "退出", Img: "cancel.png"},
		    ]);
		 ...
		 myToolbar.attachEvent("onClick", function(id){
			   if(id == 'close'){
				   ...
				   var div = window.parent.document.getElementById("editCustomer");
				   div.innerHTML = '';
		 ...
		 function buttonEvent(myLayout){
			...
			toolbar.attachEvent("onClick", function(id){
			    ...
			    if(id == 'saves'){
			    	var cCusCode = localStorage.customerID;
			    	if(cCusCode !=''){
			    		var data = new Array();
		 ...
		 function PsnSelect(e,name){
			   ...
			   tool.addButton('localization',2, '定位', null, null);
...
<body>
<div id = "toolbar" style="height:8%;width:100%;background: #F8F8F8;"></div>
<div id="postBar" style="height:90%;margin-top:1%;width:99%;display:flex;overflow:auto;">
	<div id ="seeBox" style="width:83%;height:2770px;margin-left:12%;margin-bottom:3%;background: #ffffff;box-shadow: 0 0 10px #000000;">
		<div id = "see" style="width:94%;margin-left:3%;margin-right:4%;margin-top:40px;height:2690px;">	
		</div>
	</div>
</div>
</body>
